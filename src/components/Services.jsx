...
function ServiceCard({ service, index, inView, onOpen }) {
  ...
  const hasVideoBg = !!service.bgVideoId;
  const hasImageBg = !!service.bgImage;
  const hasMedia = hasVideoBg || hasImageBg;

  // Wrapper : motion.div if iframe is involved (invalid inside <button>),
  // motion.button otherwise. The image-only background can stay in a button.
  const Wrapper = hasVideoBg ? motion.div : motion.button;
  const wrapperProps = hasVideoBg
    ? {
        role: 'button',
        tabIndex: 0,
        onClick: onOpen,
  ...
  return (
    ...
    >
      <div className={`bezel-core flex h-full flex-col p-7 transition-colors duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] md:p-9 ${isFeature ? 'lg:p-12' : ''} ${!hasMedia ? 'hover:bg-ink-800' : ''}`}>
        {/* Background : looping muted YouTube video */}
        {hasVideoBg && (
          <>
            <iframe
              aria-hidden
              src={`https://www.youtube-nocookie.com/embed/${service.bgVideoId}?autoplay=1&mute=1&loop=1&playlist=${service.bgVideoId}&controls=0&showinfo=0&modestbranding=1&playsinline=1&rel=0&iv_load_policy=3&disablekb=1`}
        ...
        {/* Background : static image (no video) */}
        {hasImageBg && !hasVideoBg && (
          <>
            <img
              aria-hidden
              src={service.bgImage}
              alt=""
              loading="lazy"
              className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-50 transition-all duration-700 group-hover:opacity-65 group-hover:scale-[1.04]"
            />
            <div aria-hidden className="pointer-events-none absolute inset-0 bg-gradient-to-tr from-ink-950/95 via-ink-950/65 to-ink-950/55" />
            <div aria-hidden className="pointer-events-none absolute inset-0" style={{
              background: 'radial-gradient(ellipse at top right, rgba(93,195,215,0.12), transparent 60%)',
        ...
        {/* Hover spotlight only on cards without media bg */}
        {!hasMedia && (
